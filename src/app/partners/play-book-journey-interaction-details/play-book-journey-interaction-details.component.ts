...
var $: any;
@Component({
  selector: 'app-play-book-journey-interaction-details',
  templateUrl: './play-book-journey-interaction-details.component.html',
  styleUrls: ['./play-book-journey-interaction-details.component.css']
})
export class PlayBookJourneyInteractionDetailsComponent implements OnInit {
  ...
  scrollClass: string;
  headerText = "Playbook Detailed Report";
  ...
  showAnalytics(item: any) {
    let route = "/home/playbook/partnerAnalytics/"+item.id + "/" + item.partnerCompanyId;
  ...
  downloadPlaybookJourneyReport() {
    ...
    let url = this.authenticationService.REST_URL + "partner/playbook/journey/interaction/details/download?access_token=" + this.authenticationService.access_token
      + "&loggedInUserId=" + loggedInUserIdRequestParam + "&searchKey=" + searchKeyRequestParm + "&detailedAnalytics=" + this.isDetailedAnalytics + "&partnerCompanyId=" + partnerCompanyIdRequestParam
