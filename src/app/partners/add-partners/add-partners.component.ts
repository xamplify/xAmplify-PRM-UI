...
@Component({
  ...
  styleUrls: [
    "./add-partners.component.css",
    "../../contacts/add-contacts/add-contacts.component.css",
...
export class AddPartnersComponent implements OnInit, OnDestroy {
  ...
  filePreview = false;
  @ViewChild("sendCampaignComponent")
  sendCampaignComponent: SendCampaignsComponent;
  ...
  constructor(
    ...
    public editContactComponent: EditContactsComponent,
    public socialPagerService: SocialPagerService,
    public manageContactComponent: ManageContactsComponent,
    ...
    if (
      this.authenticationService.companyProfileName !== undefined &&
  ...
  listPartners(userId: number) {
    try {
      this.contactService
        .listContactsOfDefaultPartnerList(userId, this.pagination)
        .subscribe(
          (data: any) => {
            this.pagination.totalRecords = data.totalRecords;
          },
          (error) => () => console.log("loadPartner() finished")
        );
    } catch (error) {
  ...
  defaultPartnerList(userId: number) {
    ...
    try {
      this.contactService.defaultPartnerList(userId).subscribe(
        (data: any) => {
  ...
  validateEmail(emailId: string) {
    ...
    if (
      lowerCaseEmail != this.editingEmailId &&
      this.contactService.allPartners != undefined
    ) {
  ...
  downloadEmptyCsv() {
    window.location.href =
      ...
      "userlists/download-default-list/" +
  ...
  savePartnerUsers() {
    ...
    var valueArr = this.newPartnerUser.map(function (item) {
    ...
    if (this.newPartnerUser[0].emailId != undefined) {
      if (!isDuplicate && !this.isEmailExist) {
        ...
        let message =
          "These " +
          this.authenticationService.partnerModule.customName +
          "(s) are already added " +
          this.existedEmailIds;
  ...
  validateSocialContacts(socialUsers: any) {
    ...
    for (let i = 0; i < socialUsers.length; i++) {
      if (socialUsers[i].emailId) {
        if (
          socialUsers[i].emailId !== null &&
          this.validateEmailAddress(socialUsers[i].emailId)
        ) {
        ...
        if (
          socialUsers[i].email !== null &&
          this.validateEmailAddress(socialUsers[i].email)
        ) {
    ...
    try {
      ...
      if (emails.length > this.teamMembersList.length) {
        for (let i = 0; i < emails.length; i++) {
          ...
          if (isEmail) {
            existedEmails.push(this.teamMembersList[i]);
          }
        }
      } else {
      ...
      if (existedEmails.length === 0) {
        if (this.isCompanyDetails) {
          if (this.validCsvContacts) {
            ...
            this.customResponse = new CustomResponse(
              "ERROR",
              "We Found Invalid emailId(s) Please remove " +
        ...
        let message =
          " You are not allowed to add teamMember(s) or yourself as a " +
          this.authenticationService.partnerModule.customName;
        this.customResponse = new CustomResponse("ERROR", message, true);
  ...
  openAssignContactAndMdfAmountPopup() {
    ...
    this.authenticationService
      .findNotifyPartnersOption(this.companyId)
      .subscribe(
        (response) => {
  ...
  validatePartnership() {
    this.processingPartnersLoader = true;
    this.contactService
      .validatePartners(this.partnerListId, this.newPartnerUser)
      .subscribe(
        (data: any) => {
          this.processingPartnersLoader = false;
  ...
  savePartners() {
    ...
    this.loading = true;
    this.contactService
      .updateContactList(this.partnerListId, this.newPartnerUser)
      .subscribe(
        (data: any) => {
          if (data.access) {
            this.loading = false;
            ...
            if (
              this.authenticationService.loggedInUserRole === "Team Member" &&
            ...
            if (data.statusCode == 200) {
              let message =
                "Your " +
                this.authenticationService.partnerModule.customName +
                "(s) have been saved successfully" +
                "<br><br>";
              ...
              if (invalidEmailIds != undefined && invalidEmailIds.length > 0) {
                ...
                if (invalidEmailIds.length == 1) {
                  message +=
                    "Following email address is not saved because it is not formatted properly" +
                    "<br><br>" +
                    allEmailIds;
                } else {
                  message +=
                    "Following email addresses are not saved because they not formatted properly" +
                    "<br><br>" +
                    allEmailIds;
                }
              }
              ...
              this.customResponse = new CustomResponse(
                "ERROR",
                data.detailedResponse[0].message,
                true
              );
              ...
              if (invalidEmailIds.length == 1) {
                message =
                  "Following email address is not formatted properly " +
                  "<br><br>" +
                  allEmailIds;
              } else {
                message =
                  "Following email addresses are not formatted properly " +
                  "<br><br>" +
                  allEmailIds;
              }
              this.customResponse = new CustomResponse("ERROR", message, true);
        ...
        (error: any) => {
          ...
          if (
            error._body.includes(
              "Please launch or delete those campaigns first"
            )
          ) {
            this.customResponse = new CustomResponse(
              "ERROR",
        ...
        () =>
          this.xtremandLogger.info(
            "MangePartnerComponent loadPartners() finished"
          )
      );
    this.dublicateEmailId = false;
  }
  ...
  cancelPartners() {
    ...
    $("#SgearIcon").attr(
      "style",
      "opacity: 1;position: relative;font-size: 19px;top: -81px;left: 71px;"
    );
    $("#GgearIcon").attr(
      "style",
      "opacity: 1;position: relative;font-size: 19px;top: -81px;left: 71px;"
    ...
    $("#copyFromClipBoard").attr(
      "style",
      "-webkit-filter: grayscale(0%);filter: grayscale(0%);"
    );
    $("#uploadCSV").attr(
      "style",
      "-webkit-filter: grayscale(0%);filter: grayscale(0%);min-height:85px;border-radius: 3px"
    );
  ...
  loadPartnerList(pagination: Pagination) {
    try {
      ...
      this.contactService
        .loadUsersOfContactList(this.partnerListId, pagination)
        .subscribe(
          (data: any) => {
            ...
            var contactIds = this.pagination.pagedItems.map(function (a) {
              return a.id;
            });
            var items = $.grep(
            ...
            if (
              ...
              items.length == this.pagination.pagedItems.length
            ) {
          ...
          () =>
            this.xtremandLogger.info(
              "MangePartnerComponent loadPartnerList() finished"
            )
        );
    } catch (error) {
  ...
  loadAllPartnerInList(totalRecords: number) {
    try {
      ...
      this.contactService
        .loadUsersOfContactList(this.partnerListId, this.allPartnersPagination)
        .subscribe(
          (data: any) => {
          ...
          () =>
            this.xtremandLogger.info(
              "MangePartnerComponent loadAllPartnerList() finished"
            )
        );
    } catch (error) {
      this.xtremandLogger.error(
  ...
  readFiles(files: any, index = 0) {
    if (this.fileUtil.isCSVFile(files[0])) {
      ...
      var outputstring = files[0].name.substring(
        0,
      ...
      $("#addContacts").attr(
        "style",
        "-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
      );
      $("#copyFromClipBoard").attr(
        "style",
        "-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
      );
      $(".salesForceImageClass").attr(
        "style",
        "opacity: 0.5;-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
      );
      $(".googleImageClass").attr(
        "style",
        "opacity: 0.5;-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
      );
      $(".zohoImageClass").attr(
      ...
      $(".marketoImageClass").attr(
        "style",
        "opacity: 0.5;-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
      );
      $(".hubspotImageClass").attr(
        "style",
        "opacity: 0.5;-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
      );
      $("#SgearIcon").attr(
        "style",
        "opacity: 0.5;position: relative;top: -81px;left: 71px;-webkit-filter: grayscale(100%);filter: grayscale(100%);"
      );
      $("#GgearIcon").attr(
        "style",
        "opacity: 0.5;position: relative;top: -86px; left: 71px;-webkit-filter: grayscale(100%);filter: grayscale(100%);"
      );
      $("#ZgearIcon").attr(
        "style",
        "opacity: 0.5;position: relative;top: -81px;left: 71px;-webkit-filter: grayscale(100%);filter: grayscale(100%);"
  ...
  validateHeaders(headers) {
    return (
      ...
      headers[2].trim() == "COMPANY" &&
      headers[3].trim() == "JOBTITLE" &&
      headers[4].trim() == "EMAILID" &&
      headers[5].trim() == "VERTICAL" &&
      headers[6].trim() == "REGION" &&
      headers[7].trim() == "TYPE" &&
      headers[8].trim() == "CATEGORY" &&
      headers[9].trim() == "ADDRESS" &&
      headers[10].trim() == "CITY" &&
      headers[11].trim() == "STATE" &&
      headers[12].trim() == "ZIP" &&
      headers[13].trim() == "COUNTRY" &&
      headers[14].trim() == "MOBILE NUMBER"
    );
  }
  ...
  copyFromClipboard() {
    ...
    $("#uploadCSV").attr(
      "style",
      "-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;min-height:85px;border-radius: 3px"
    );
    $("#addContacts").attr(
      "style",
      "-webkit-filter: grayscale(100%);filter: grayscale(100%);cursor:not-allowed;"
  ...
  clipboardShowPreview() {
    ...
    if (selectedDropDown == "DEFAULT") {
      swal(
        "<span style='font-weight: 100;font-family: Open Sans;font-size: 16px;'>Please Select the Delimiter Type</span>"
      );
      return false;
    } else {
      if (selectedDropDown == "CommaSeperated") splitValue = ",";
      else if (selectedDropDown == "TabSeperated") splitValue = "\t";
    }
    ...
    if (this.clipboardTextareaText === "") {
      $("#clipBoardValidationMessage").append(
        "<h4 style='color:#f68a55;'>" + "Please enter the valid data." + "</h4>"
      );
      isValidData = false;
    }
    ...
    if (this.clipboardTextareaText != "") {
      for (var i = 0; i < allTextLines.length; i++) {
        ...
        if (!this.validateEmailAddress(data[4])) {
          $("#clipBoardValidationMessage").append(
            "<h4 style='color:#f68a55;'>" +
              "Email Address is not valid for Row:" +
              (i + 1) +
    ...
    if (isValidData) {
      ...
      $("#clipBoardValidationMessage").append(
        "<h5 style='color:#07dc8f;'><i class='fa fa-check' aria-hidden='true'></i>" +
          "Processing started at: <b>" +
          startTime +
          "</b></h5>"
      );
      $("#clipBoardValidationMessage").append(
        "<h5 style='color:#07dc8f;'><i class='fa fa-check' aria-hidden='true'></i>" +
          "Processing Finished at: <b>" +
          endTime +
          "</b></h5>"
      );
      $("#clipBoardValidationMessage").append(
        "<h5 style='color:#07dc8f;'><i class='fa fa-check' aria-hidden='true'></i>" +
          "Total Number of records Found: <b>" +
          allTextLines.length +
          "</b></h5>"
      );
    } else {
  ...
  removeContactListUsers1(contactId: number) {
    try {
      ...
      this.contactService
        .removeContactListUsers(this.partnerListId, this.partnerId)
        .subscribe(
          (data: any) => {
            if (data.access) {
              let message =
                "Your " +
                this.authenticationService.partnerModule.customName +
                "(s) have been deleted successfully";
              this.customResponse = new CustomResponse(
                "SUCCESS",
                message,
                true
  ...
  updatePartnerListUser(event) {
    try {
      ...
      this.addPartnerModalClose();
      this.contactService
        .updateContactListUser(this.partnerListId, this.editUser)
        .subscribe(
          (data: any) => {
            if (data.access) {
              let message =
          ...
          () =>
            this.xtremandLogger.info(
              "EditPartnerComponent updateContactListUser() finished"
            )
        );
    } catch (error) {
  ...
  socialContactImage() {
    try {
      this.contactService.socialContactImages().subscribe(
        (data) => {
        ...
        () =>
          this.xtremandLogger.log(
            "AddContactsComponent socialContactImage() finished."
          )
      );
    } catch (error) {
      this.xtremandLogger.error(
  ...
  setSocialPage(page: number) {
    try {
      ...
      if (this.paginationType == "csvPartners") {
        this.pager = this.socialPagerService.getPager(
          this.newPartnerUser.length,
          page,
          this.pageSize
        );
        this.pagedItems = this.newPartnerUser.slice(
          this.pager.startIndex,
          this.pager.endIndex + 1
        );
      } else {
        this.pager = this.socialPagerService.getPager(
          this.socialPartnerUsers.length,
          page,
          this.pageSize
        );
        this.pagedItems = this.socialPartnerUsers.slice(
          this.pager.startIndex,
          this.pager.endIndex + 1
        );

        var contactIds = this.pagedItems.map(function (a) {
          return a.id;
        });
        ...
        if (
          items.length == this.pager.pageSize ||
          items.length == this.getGoogleConatacts.length ||
          items.length == this.pagedItems.length
        ) {
    ...
    try {
      if (this.loggedInThroughVanityUrl) {
        ...
        if (
          ...
          this.xtremandLogger.info(
            "socialContacts" + this.socialPartners.socialNetwork
          );
          this.contactService.googleLogin("partners").subscribe(
            (response) => {
            ...
            (error: any) => {
              this.xtremandLogger.error(error);
              if (
                error._body.includes("JSONObject") &&
                error._body.includes("access_token") &&
                error._body.includes("not found.")
              ) {
            ...
            () =>
              this.xtremandLogger.log(
                "AddContactsComponent googleContacts() finished."
              )
          );
        }
      }
  ...
  getGoogleContactsUsers() {
    try {
      let partnerModuleCustomName = localStorage.getItem(
        "partnerModuleCustomName"
      );
      let message =
        "Retrieving " +
        partnerModuleCustomName +
        " from google...! Please Wait...It's processing";
      swal({
        text: message,
      ...
      this.socialPartners.socialNetwork = "GOOGLE";
      var self = this;
      this.contactService.getGoogleContacts(this.socialPartners).subscribe(
        (data) => {
          ...
          if (!this.getGoogleConatacts.contacts) {
            this.customResponse = new CustomResponse(
              "ERROR",
              this.properties.NO_RESULTS_FOUND,
              true
            );
            this.selectedAddPartnerOption = 5;
          } else {
            for (var i = 0; i < this.getGoogleConatacts.contacts.length; i++) {
              ...
              if (
                this.validateEmailAddress(
                  this.getGoogleConatacts.contacts[i].emailId
                )
              ) {
                socialContact.emailId =
                  this.getGoogleConatacts.contacts[i].emailId.trim();
                socialContact.firstName =
                  this.getGoogleConatacts.contacts[i].firstName;
                socialContact.lastName =
                  this.getGoogleConatacts.contacts[i].lastName;
        ...
        () =>
          this.xtremandLogger.log(
            "googleContacts data :" +
              JSON.stringify(this.getGoogleConatacts.contacts)
          )
      );
    } catch (error) {
      this.xtremandLogger.error(
  ...
  saveGoogleContacts() {
    ...
    if (this.socialPartnerUsers.length > 0) {
      ...
      this.xtremandLogger.error(
        "ManagePartnerComponent saveGoogleContacts() Contacts Null Error"
      );
  }
  ...
  saveGoogleContactSelectedUsers() {
    ...
    if (this.allselectedUsers.length != 0) {
      ...
      this.xtremandLogger.error(
        "ManagePartnerComponent saveGoogleContactSelectedUsers() ContactListName Error"
      );
    }
  }
  ...
  zohoContacts() {
    try {
      this.fileTypeError = false;
      let self = this;
      self.selectedZohoDropDown = $(
        "select.opts:visible option:selected "
      ).val();
  ...
  getZohoContacts(contactType: any, username: string, password: string) {
    try {
      ...
      var self = this;
      this.contactService
        .getZohoContacts(this.userName, this.password, this.contactType)
        .subscribe(
          (data) => {
            this.getGoogleConatacts = data;
            ...
            if (!this.getGoogleConatacts.contacts) {
              ...
              for (
                var i = 0;
                i < this.getGoogleConatacts.contacts.length;
                i++
              ) {
                let socialContact = new SocialContact();
                let user = new User();
          ...
          (error: any) => {
            ...
            if (body != "") {
              ...
              if (
                response.message ==
                "Maximum allowed AuthTokens are exceeded, Please remove Active AuthTokens from your ZOHO Account.!"
              ) {
                this.zohoCredentialError =
                  "Maximum allowed AuthTokens are exceeded, Please remove Active AuthTokens from your ZOHO Account.!";
                setTimeout(() => {
  ...
  authorisedZohoContacts() {
    try {
      ...
      this.contactService
        .getZohoAutherizedContacts(this.socialPartners)
        .subscribe(
          (data) => {
            this.getGoogleConatacts = data;
  ...
  saveZohoContacts() {
    ...
    if (this.socialPartnerUsers.length > 0) {
      ...
      this.xtremandLogger.error(
        "AddContactComponent saveZohoContacts() Contacts Null Error"
      );
    localStorage.removeItem("isZohoSynchronization");
  }
  ...
  saveZohoContactSelectedUsers() {
    ...
    if (this.allselectedUsers.length != 0) {
      ...
      this.xtremandLogger.error(
        "AddContactComponent saveZohoContactSelectedUsers() ContactList Name Error"
      );
      localStorage.removeItem("isZohoSynchronization");
    }
  }
  ...
  onChange(item: any) {
    ...
    for (var i = 0; i < this.salesforceListViewsData.length; i++) {
      ...
      if (item == this.salesforceListViewsData[i].listViewId) {
        this.salesforceListViewName =
          this.salesforceListViewsData[i].listViewName;
      }
      this.xtremandLogger.log(
        "listviewNameDROPDOWN" + this.salesforceListViewName
      );
    }
  }
  ...
  onChangeSalesforceDropdown(event: Event) {
    try {
      ...
      if (
        this.contactType == "contact_listviews" ||
        this.contactType == "lead_listviews"
      ) {
        $("button#salesforce_save_button").prop("disabled", true);
        this.contactService
          .getSalesforceContacts(this.socialNetwork, this.contactType)
          .subscribe(
            (data) => {
    ...
    try {
      if (this.loggedInThroughVanityUrl) {
        ...
        if (
          ...
          this.contactService.salesforceLogin("partners").subscribe(
            (response) => {
              this.storeLogin = response.data;
              let data = response.data;
              if (response.statusCode == 200) {
            ...
            () =>
              this.xtremandLogger.log(
                "addContactComponent salesforceContacts() login finished."
              )
          );
        }
      }
  ...
  getSalesforceContacts(contactType: any) {
    try {
      ...
      if (selectedDropDown == "DEFAULT") {
        ...
        this.xtremandLogger.log(
          "AddContactComponent getSalesforceContacts() selected Dropdown value:" +
            this.contactType
        );
      }
      ...
      this.contactService
        ...
        .subscribe(
          (data) => {
            ...
            if (!this.getGoogleConatacts.contacts) {
              if (
                this.getGoogleConatacts.jsonData.includes(
                  "API_DISABLED_FOR_ORG"
                )
              ) {
                this.customResponse = new CustomResponse(
                  "ERROR",
                  "Salesforce REST API is not enabled, Please change your Salesforce platform settings.",
                  true
                );
              } else {
                this.customResponse = new CustomResponse(
          ...
          () =>
            this.xtremandLogger.log(
              "addContactComponent getSalesforceContacts() Data:" +
                JSON.stringify(this.getGoogleConatacts.contacts)
            )
        );
    } catch (error) {
  ...
  getSalesforceListViewContacts(contactType: any) {
    try {
      ...
      this.contactService
        .getSalesforceListViewContacts(
          this.socialNetwork,
          this.contactType,
          this.salesforceListViewId,
          this.salesforceListViewName
        )
        .subscribe(
          (data) => {
  ...
  saveSalesforceContactSelectedUsers() {
    ...
    if (this.allselectedUsers.length != 0) {
      this.xtremandLogger.info(
        "update contacts #contactSelectedListId " +
          " data => " +
          JSON.stringify(this.allselectedUsers)
      );
      this.newPartnerUser = this.allselectedUsers;
      this.saveValidEmails();
    } else {
      this.xtremandLogger.error(
        "AddContactComponent saveSalesforceContactSelectedUsers() ContactList Name Error"
      );
    }
  }
  ...
  saveSalesforceContacts() {
    ...
    if (this.socialPartnerUsers.length > 0) {
      ...
      this.xtremandLogger.error(
        "AddContactComponent saveSalesforceContacts() Contacts Null Error"
      );
  }
  ...
  proceed() {
    ...
    if (this.selectedAddPartnerOption == 3) {
      ...
      } else this.saveGoogleContactSelectedUsers();
    }

    if (this.selectedAddPartnerOption == 6) {
      if (this.allselectedUsers.length == 0) {
        this.saveZohoContacts();
      } else this.saveZohoContactSelectedUsers();
    }

    if (this.selectedAddPartnerOption == 7) {
      if (this.allselectedUsers.length == 0) {
        this.saveSalesforceContacts();
      } else this.saveSalesforceContactSelectedUsers();
    }
  ...
  validateLegalBasisOptions() {
    ...
    if (
      this.selectedAddPartnerOption != 1 &&
      this.gdprStatus &&
      this.selectedLegalBasisOptions.length == 0
    ) {
  ...
  unlinkSocailAccount() {
    try {
      ...
      this.contactService
        .unlinkSocailAccount(socialNetwork, this.isUnLinkSocialNetwork)
        .subscribe(
          (data: any) => {
            ...
            this.customResponse = new CustomResponse(
              "SUCCESS",
              this.properties.SOCIAL_ACCOUNT_REMOVED_SUCCESS,
              true
            );
          },
          (error: any) => {
      ...
      this.xtremandLogger.error(
        error,
        "addPartnerComponent",
        "unlink social accounts Partners"
      );
    }
  }
  ...
  checkAll(ev: any) {
    if (
      ...
      if (ev.target.checked) {
        ...
        this.allselectedUsers = this.removeDuplicates(
          this.allselectedUsers,
          "emailId"
        );
        this.selectedContactListIds = this.referenceService.removeDuplicates(
        ...
        if (this.pager.maxResults == this.totalRecords) {
          ...
          for (let j = 0; j < this.pagedItems.length; j++) {
            ...
            this.allselectedUsers =
              this.referenceService.removeRowsFromPartnerOrContactListByEmailId(
                this.allselectedUsers,
                paginationEmail
              );
          }
          let currentPageContactIds = this.pagedItems.map(function (a) {
            return a.id;
          });
          this.selectedContactListIds =
            this.referenceService.removeDuplicatesFromTwoArrays(
              this.selectedContactListIds,
  ...
  highlightRow(
    ...
    if (isChecked) {
      ...
      this.selectedContactListIds.splice(
        $.inArray(contactId, this.selectedContactListIds),
        1
      );
  ...
  hasAccess() {
    try {
      return this.contactService.hasAccess(this.isPartner).map((data) => {
  ...
  downloadPartnerList() {
    this.hasAccess().subscribe((data) => {
      if (data) {
        try {
          this.downloadAssociatedPagination.userListId = this.partnerListId;
          this.downloadAssociatedPagination.userId =
            this.authenticationService.getUserId();
          if (
            this.isPartner &&
            this.authenticationService.loggedInUserRole === "Team Member" &&
            ...
            this.referenceService.setTeamMemberFilterForPagination(
              this.downloadAssociatedPagination,
              this.selectedFilterIndex
            );
          }
          this.contactService
            .downloadContactList(this.downloadAssociatedPagination)
            .subscribe(
  ...
  sendMail(partnerId: number) {
    try {
      ...
      this.contactService.mailSend(this.pagination).subscribe(
        (data) => {
          if (data.access) {
            this.loading = false;
            this.customResponse = new CustomResponse(
              "SUCCESS",
              this.properties.EMAIL_SENT_SUCCESS,
              true
            );
        ...
        () =>
          this.xtremandLogger.log(
            "Manage Partner component Mail send method successfull"
          )
      );
    } catch (error) {
      this.xtremandLogger.error(
  ...
  selectedPageNumber(event) {
    ...
    if (event === 0) {
      event = this.socialPartnerUsers.length;
    }
    this.pageSize = event;
    this.setSocialPage(1);
  }
  ...
  getContactsAssocialteCampaigns() {
    try {
      this.contactService
        .contactListAssociatedCampaigns(
          this.partnerListId,
          this.contactAssociatedCampaignPagination
        )
        .subscribe(
          (data) => {
  ...
  closeModal(event) {
    if (event === "Emails Send Successfully") {
      let message =
        "Your " +
        this.authenticationService.partnerModule.customName +
        " list has been updated successfully and any selected campaigns have been launched";
    ...
    if (event === "users are unSubscribed for emails") {
      this.customResponse = new CustomResponse(
        "ERROR",
        "The partners are unsubscribed for receiving the campaign emails.",
        true
      );
    }
    ...
    if (event === "user has unSubscribed for emails") {
      this.customResponse = new CustomResponse(
        ...
        "The " +
          this.authenticationService.partnerModule.customName +
          " has unsubscribed for receiving the campaign emails.",
        true
      );
    }
    ...
    try {
      ...
      if ($.inArray(inputName, names) > -1) {
        ...
        if (this.saveAsListName !== "" && this.saveAsListName.length < 250) {
          this.editContactComponent.saveDuplicateContactList(
            this.saveAsListName,
            [],
            true
          );
  ...
  ngAfterViewChecked() {
    ...
    if (tempCheckGoogleAuth == "yes") {
      ...
      tempValidationMessage != null &&
      tempValidationMessage.length > 0 &&
      this.isPartner
    ) {
      swal.close();
      this.customResponse = new CustomResponse(
      ...
      window.addEventListener(
        ...
        function (e) {
          ...
          if (e.data == "isGoogleAuth") {
            ...
            e.data != null &&
            e.data.includes("You have already configured")
          ) {
  ...
  checkTermsAndConditionStatus() {
    if (this.companyId > 0) {
      this.loading = true;
      this.userService.getGdprSettingByCompanyId(this.companyId).subscribe(
        (response) => {
          if (response.statusCode == 200) {
            this.gdprSetting = response.data;
            this.gdprStatus = this.gdprSetting.gdprStatus;
            this.termsAndConditionStatus =
              this.gdprSetting.termsAndConditionStatus;
          }
          this.parentInput["termsAndConditionStatus"] =
            this.termsAndConditionStatus;
  ...
  getLegalBasisOptions() {
    if (this.companyId > 0) {
      ...
      this.referenceService.getLegalBasisOptions(this.companyId).subscribe(
        (data) => {
    ...
    if (this.loggedInThroughVanityUrl) {
      ...
      try {
        if (
          ...
          this.contactService
            .checkMarketoCredentials(this.authenticationService.getUserId())
            .subscribe(
              (data: any) => {
              ...
              () =>
                this.xtremandLogger.info(
                  "Add contact component loadContactListsName() finished"
                )
            );
        }
      } catch (error) {
        this.xtremandLogger.error(
          error,
          "AddContactsComponent zohoContactsAuthenticationChecking()."
        );
      }
    }
  }
  ...
  saveMarketoContacts() {
    ...
    if (this.socialPartnerUsers.length > 0) {
      ...
      this.xtremandLogger.error(
        "AddContactComponent saveMarketoContacts() Contacts Null Error"
      );
  }
  ...
  saveMarketoContactSelectedUsers() {
    ...
    if (this.allselectedUsers.length != 0) {
      ...
      this.xtremandLogger.error(
        "AddContactComponent saveMarketoContactSelectedUsers() ContactList Name Error"
      );
    }
  }
  ...
  retriveMarketoContacts() {
    ...
    this.contactService
      .getMarketoContacts(this.authenticationService.getUserId())
      .subscribe(
        (data) => {
          if (data.statusCode === 200) {
            this.selectedAddPartnerOption = 8;
            ...
            if (this.getMarketoConatacts.length == 0) {
              ...
              for (var i = 0; i < this.getMarketoConatacts.length; i++) {
                ...
                socialPartner.id = i;
                if (
                  this.validateEmailAddress(this.getMarketoConatacts[i].email)
                ) {
                  socialPartner.emailId = this.getMarketoConatacts[i].email;
                  socialPartner.firstName =
                    this.getMarketoConatacts[i].firstName;
                  ...
                  socialPartner.postalCode =
                    this.getMarketoConatacts[i].postalCode;
                  ...
                  socialPartner.mobilePhone =
                    this.getMarketoConatacts[i].mobilePhone;
        ...
        () =>
          this.xtremandLogger.log(
            "marketoContacts data :" + JSON.stringify(this.getMarketoConatacts)
          )
      );
  }
  ...
  getMarketoContacts() {
    ...
    this.contactService.saveMarketoCredentials(obj).subscribe(
      (response) => {
  ...
  toggleMarketoSubmitButtonState() {
    if (
      !this.marketoClentIdError &&
      !this.marketoSecretIdError &&
      !this.marketoInstanceError
    )
      this.isMarketoModelFormValid = true;
    else this.isMarketoModelFormValid = false;
  }
  ...
  highlightMarketoRow(user: any, event, index) {
    ...
    if (isChecked) {
      ...
      this.selectedContactListIds.splice(
        $.inArray(user.id, this.selectedContactListIds),
        1
      );
  ...
  checkingHubSpotContactsAuthentication() {
    if (this.loggedInThroughVanityUrl) {
      ...
      if (this.selectedAddPartnerOption == 5) {
        this.hubSpotService.configHubSpot().subscribe(
          (data) => {
            let response = data;
            if (
              response.data.isAuthorize !== undefined &&
              response.data.isAuthorize
            ) {
          ...
          (error: any) => {
            this.xtremandLogger.error(
              error,
              "Error in HubSpot checkIntegrations()"
            );
          },
        ...
        if (
          ...
          if (
            ...
            window.open(
              url,
              "Social Login",
              "toolbar=yes,scrollbars=yes,addressbar=noresizable=yes,top=" +
                y +
                ",left=" +
                x +
  ...
  onChangeHubSpotDropdown(event: Event) {
    try {
      ...
      if (this.contactType === "lists") {
        $("button#hubspot_save_button").prop("disabled", true);
        this.hubSpotService.getHubSpotContactsLists().subscribe(
          (data) => {
      ...
      this.xtremandLogger.error(
        error,
        "AddContactsComponent onChangeHubSpotDropdown()."
      );
    }
  }
  ...
  getHubSpotContactsListsById() {
    this.xtremandLogger.info(
      "hubSpotSelectContactListOption :" + this.hubSpotSelectContactListOption
    );
    if (
      this.hubSpotSelectContactListOption !== undefined &&
      this.hubSpotSelectContactListOption !== ""
    ) {
      this.hubSpotService
        .getHubSpotContactsListsById(this.hubSpotSelectContactListOption)
        .subscribe((data) => {
          let response = data.data;
          this.selectedAddPartnerOption = 9;
          this.frameHubSpotFilePreview(response);
  ...
  saveHubSpotContactSelectedUsers() {
    ...
    if (this.allselectedUsers.length != 0) {
      ...
      this.xtremandLogger.error(
        "AddContactComponent saveHubSpotContactSelectedUsers() ContactList Name Error"
      );
    }
  }
  ...
  showAlert() {
    this.xtremandLogger.info(
      "userIdForChecked" + this.editContactComponent.selectedContactListIds
    );
    if (this.editContactComponent.selectedContactListIds.length != 0) {
      this.xtremandLogger.info(
        "contactListId in sweetAlert() " + this.partnerListId
      );
      let self = this;
      swal({
        title: "Are you sure?",
    ...
    if (this.gdprStatus) {
      ...
      $.each(list, function (index, contact) {
        if (self.legalBasisOptions.length > 0) {
          let filteredLegalBasisOptions = $.grep(
            self.legalBasisOptions,
            function (e) {
              return contact.legalBasis.indexOf(e.id) > -1;
            }
          );
          let selectedLegalBasisOptionsArray = filteredLegalBasisOptions.map(
            function (a) {
              return a.name;
            }
          );
  ...
  addCampaigns(contact: any) {
    this.sendCampaignComponent.openPopUp(
      this.partnerListId,
      contact,
      "Partner"
    );
    ...
    try {
      ...
      if (this.loggedInThroughVanityUrl) {
        ...
        if (
          ...
          this.contactService.checkingZohoAuthentication(this.module).subscribe(
            (response: any) => {
              let data = response.data;
              this.storeLogin = data;
              if (response.statusCode == 200) {
            ...
            () =>
              this.xtremandLogger.info(
                "Add contact component checkingZohoContactsAuthentication() finished"
              )
          );
        }
      }
        ...
        (response: any) => {
          ...
          if (response.statusCode == 200) {
            ...
            window.open(
              url,
              "Social Login",
              "toolbar=yes,scrollbars=yes,resizable=yes, addressbar=no,top=" +
                y +
                ",left=" +
                x +
                ",width=700,height=485"
  ...
  getZohoLeadsUsingOAuth2() {
    ...
    this.contactService.getZohoAutherizedLeads(this.socialPartners).subscribe(
      (data) => {
        this.selectedAddPartnerOption = 6;
        if (data.statusCode != null && data.statusCode != 200) {
          swal.close();
  ...
  processZohoContactsToDisplayInUI(data) {
    ...
    if (contacts != null && contacts.length > 0) {
      for (var i = 0; i < this.getGoogleConatacts.contacts.length; i++) {
        ...
        socialContact.id = i;
        if (
          this.validateEmailAddress(this.getGoogleConatacts.contacts[i].emailId)
        ) {
          socialContact.emailId =
            this.getGoogleConatacts.contacts[i].emailId.trim();
          socialContact.firstName =
            this.getGoogleConatacts.contacts[i].firstName;
          socialContact.lastName = this.getGoogleConatacts.contacts[i].lastName;
          socialContact.company =
            this.getGoogleConatacts.contacts[i].contactCompany;
          socialContact.contactCompany =
            this.getGoogleConatacts.contacts[i].contactCompany;
          this.socialPartnerUsers.push(socialContact);
        }
      }
      ...
      (error: any) => {
        ...
        if (
          ...
          this.xtremandLogger.errorMessage =
            "authentication was not successful, you might have changed the password of social account or other reasons, please unlink your account and reconnect it.";
        }
        this.xtremandLogger.errorPage(error);
      },
      () =>
  ...
  onChangeZohoDropdown(event: Event) {
    try {
      ...
      this.xtremandLogger.error(
        error,
        "AddContactsComponent SalesforceContactsDropdown()."
      );
    }
  }
