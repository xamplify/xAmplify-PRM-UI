...
iframeLeadContent: any = `<!DOCTYPE html>
<html lang="en" xmlns="http://www.w3.org/1999/xhtml">
...
<body>
  <div class="main-wrapper">
    ...
    <div class="white-card">
      <div style="widht:100%;height:99%" id="leadProgression-bar-chart"></div>
     <div class="asset-summary-container">
          <h2>{{report.statusChangeTimeline.title}}</h2>
      <table>
        <thead>
          <tr>
            <th class="text_center">Date</th>
            <th class="text_center">Stage Changed To</th>
			<th class="text_center">Changed By</th>
          </tr>
        </thead>
        <tbody>
         {{#report.statusChangeTimeline.items}}
      <tr>
        <td class="text_center">{{date}}</td>
         <td class="text_center"><span class="type-badge" style="background-color: {{background_color_hex}}; color: white;">{{stage_changed_to}}</span></td>
        <td class="text_center">{{changed_by}}</td>
      </tr>
    {{/report.statusChangeTimeline.items}}
        </tbody>
      </table>
  ...
  <script>
      Highcharts.chart('leadProgression-bar-chart', {
        chart: { type: 'column' },
        credits: { enabled: false },
        title: { text: 'Progression TimeLine' },
        xAxis: { categories: {{{report.leadProgressionTimeline.categoriesXaxisString}}},title: {
            text: null
        }},
        yAxis: { categories: {{{report.leadProgressionTimeline.categoriesYAxisString}}},title: {
            text: null
        }},
         series: [{
        name: 'Stage',
        data: {{{report.leadProgressionTimeline.seriesString}}},
    }]
    });
</script>
</body>
...
<body>
  ...
  iframeDealContent: any = `<!DOCTYPE html>
<html lang="en" xmlns="http://www.w3.org/1999/xhtml">
...
<body>
    ...
    <div class="header-card">
      <div class="header">
        ...
        {{#report}}
                <div class="owner-summary" style="margin-bottom: 15px;">
                <ul class="owner-list">
                {{#report.deal_title}}      <li><span class="label">Title</span>            <span class="value">{{report.deal_title}}</span></li>{{/report.deal_title}}
                {{#report.deal_amount}}      <li><span class="label">Amount</span>   <span class="value">{{report.deal_amount}}</span></li>{{/report.deal_amount}}
                {{#report.deal_close_date}}<li><span class="label">Close Date</span>          <span class="value">{{report.deal_close_date}}</span></li>{{/report.deal_close_date}}
                {{#report.campaign_name}}       <li><span class="label">Campaign Name</span>            <span class="value">{{report.campaign_name}}</span></li>{{/report.campaign_name}}
                {{^report.campaign_name}} <li>
             <span class="label">Created For</span>
             <span class="value">{{report.created_for_company}}</span>
  ...
  <script>
    ...
    buildIframe(): void {
        ...
        }else  if (this.agentType === 'LEADAGENT' || this.intent == 'lead') {
            iframeContentData = this.iframeLeadContent;
        }else  if (this.agentType === 'DEALAGENT' || this.intent == 'deal') {
            iframeContentData = this.iframeDealContent;
        } else {
  ...
  downloadIframeAsPDF() {
    ...
    ngAfterViewInit(): void {
      ...
      if (this.isFromGroupOfPartners || this.activeTab == 'partnergroupagent') {
        ...
        this.reportData.leadProgressionTimeline.categoriesXaxisString =
            JSON.stringify(this.reportData.leadProgressionTimeline.categoriesXaxis);
        this.reportData.leadProgressionTimeline.categoriesYAxisString =
            JSON.stringify(this.reportData.leadProgressionTimeline.categoriesYAxis);
        this.reportData.leadProgressionTimeline.seriesString =
            JSON.stringify(this.reportData.leadProgressionTimeline.series);
        }
        this.buildIframe();
    }
