...
export class TwitterService {
    ...
    getTweets(userId:number, pageSize:number):Observable<Object>{
        return this.http.get(this.URL+"tweets"+"?access_token="+this.authService.access_token+"&userId="+userId+"&pageSize="+pageSize)
            .map(this.extractData)
            .catch(this.handleError);
    }
    ...
    sendDirectMessage(toUserId:number, text:string){
        return this.http.get(this.URL+"send-direct-message"+"?access_token="+this.authService.access_token+"&toUserId="+toUserId+"&text="+text)
        .map(this.extractData)
        .catch(this.handleError);
    }
