...
var symbol = _Symbol();
var symbol_event = _Symbol();
var symbol_play = _Symbol('nativeplay');
var symbol_pause = _Symbol('nativepause');
...
function overloadAPI(video) {
	var player = video[symbol];
	video[symbol_play] = video.play;
	video[symbol_pause] = video.pause;
	video.play = play;
	video.pause = pause;
	...
	preventEvent(video, 'timeupdate', symbol_event, false);
	preventEvent(video, 'ended', symbol_event, false); // prevent occasional native ended events
}
...
function enableInlineVideo(video) {
	...
	if (onlyWhenNeeded && !isNeeded || video[symbol]) {
		return;
	}
...
var defaults = {
    ...
    showNotice: true,
    NoticeMessage: "<img src='https://mobinar.com/images/360load1.png'>",
    autoHideNotice: 3000,
